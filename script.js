...
document.querySelectorAll(".quiz").forEach((section) => {
  ...
  function render() {
    ...
    if (submitted) {
      ...
      action.innerHTML = `<div class="result"><span>Your ${type.toUpperCase()} score</span><strong>${score}<small>/8</small></strong><p>${score >= 7 ? "Excellent eye!" : score >= 5 ? "Strong performance." : "Virtual staining can be surprisingly convincing."}</p><button class="reset">Play another random round</button></div>`;
    } else
